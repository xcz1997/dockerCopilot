...
async function handleSourceChange(newSourceType, registryHost = "") {
    ...
    try {
        const result = await imagesStore.updateSource(
            id,
            newSourceType,
            registryHost,
        );
...
async function handlePrune() {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (deletedCount > 0) {
                toastStore.success(
                    `已清除 ${deletedCount} 个未使用镜像，释放 ${spaceStr}`,
                );
            } else {
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div
                ...
                :class="[
                    'card card-hover p-4 cursor-pointer transition-all',
                    filterType === 'all' && !filterUpdate
            ...
            <div
                ...
                :class="[
                    'card card-hover p-4 cursor-pointer transition-all',
                    filterType === 'used' ? 'ring-2 ring-emerald-500' : '',
                ]"
            >
                <div class="flex items-center gap-3">
                    <div
            ...
            <div
                ...
                :class="[
                    'card card-hover p-4 cursor-pointer transition-all',
                    filterType === 'unused' ? 'ring-2 ring-gray-500' : '',
                ]"
            >
                <div class="flex items-center gap-3">
            ...
            <div
                ...
                :class="[
                    'card card-hover p-4 cursor-pointer transition-all',
                    filterUpdate ? 'ring-2 ring-amber-500' : '',
                ]"
            >
                <div class="flex items-center gap-3">
                    <div
        ...
        <div class="card p-3 sm:p-4">
            <div class="flex flex-col gap-3 sm:gap-4">
                ...
                <div class="flex items-center justify-between gap-2">
                    <div
                        class="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 -mb-1 scrollbar-thin"
                    >
                        <button
                            v-for="type in [
                                { value: 'all', label: '全部' },
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div class="relative">
                            ...
                            <div
                                v-if="showSortMenu"
                                class="absolute right-0 mt-1 w-36 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-20"
                            >
                                <button
                        ...
                        >
                            <svg
                                :class="[
                                    ...
                                    imagesStore.loading && 'animate-spin',
                                ]"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
            ...
            >
                <div class="flex flex-col h-full">
                    ...
                    <div class="flex items-start gap-2 sm:gap-3 mb-3 sm:mb-4">
                        <div
                            class="p-1.5 sm:p-2 bg-gray-100 dark:bg-gray-700 rounded-lg flex-shrink-0"
                        >
                            <svg
                    ...
                    <!-- 标签组 - 移动端可横向滚动 -->
                    <div
                        class="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1.5 -mb-1.5 scrollbar-thin mb-3 sm:mb-4"
                    >
                        <span
                            :class="[
                                'badge text-xs whitespace-nowrap flex-shrink-0',
                                image.inUsed ? 'badge-success' : 'badge-gray',
                            ]"
                        >
                        ...
                        <span
                            ...
                            class="badge badge-warning text-xs whitespace-nowrap flex-shrink-0"
                        >
                            有更新
                        </span>
                        <!-- 来源标记（可点击切换） -->
                        ...
                        >
                            <svg
                                v-if="updatingSourceIds.has(image.id)"
                        ...
                        >
                            <svg
                                v-if="updatingTagIds.has(image.id)"
                                class="w-3 h-3 animate-spin mr-0.5"
                                viewBox="0 0 24 24"
                            >
                                <circle
                    ...
                    <!-- 详细信息 -->
                    <div
                        class="flex-1 space-y-1.5 sm:space-y-2 text-xs sm:text-sm"
                    >
                    ...
                    <!-- 操作按钮 -->
                    <div
                        class="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700 flex gap-1.5 sm:gap-2"
                    >
                        <button
                            v-if="image.haveUpdate"
            ...
            <div class="card w-full max-w-md p-6 animate-scale-in" @click.stop>
                ...
                <p class="text-gray-600 dark:text-gray-400 mb-4">
                    ...
                    <strong class="text-gray-900 dark:text-white">{{
                        getImageFullName(selectedImage)
                    }}</strong>
                    吗？
                </p>

                <div
                    v-if="selectedImage?.inUsed"
                    class="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg mb-4"
                >
                    <div
                        class="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-medium mb-1"
                    >
                        <svg
                ...
                <label
                    :class="[
                        ...
                        selectedImage?.inUsed ? 'opacity-60' : '',
                    ]"
                >
                    <input
                        type="checkbox"
            ...
            <div class="card w-full max-w-md p-6 animate-scale-in" @click.stop>
                ...
                <div class="flex justify-end gap-3">
                    <button
                        ...
                        class="btn btn-secondary"
                        :disabled="pullingIds.has(selectedImage?.id)"
                    >
                        取消
                    </button>
                    <button
                    ...
                    >
                        <svg
                            v-if="pullingIds.has(selectedImage?.id)"
                        ...
                        {{
                            pullingIds.has(selectedImage?.id)
                                ? "拉取中..."
                                : "确认更新"
                        }}
                    </button>
                ...
                <div class="space-y-3 mb-6">
                    ...
                    <button
                        ...
                        :class="[
                            ...
                            selectedImage?.sourceType === 'remote' ||
                            !selectedImage?.sourceType
                                ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20'
                                : 'border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-700',
                        ]"
                    >
                        <div
                            class="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex-shrink-0"
                        >
                            <svg
                        ...
                        <div>
                            ...
                            >
                                从 Docker Hub 或公共 Registry 检查更新
                            </div>
                        </div>
                        <div
                            v-if="
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <div
                                class="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex-shrink-0"
                            >
                                <svg
                            ...
                            <div class="flex-1">
                                ...
                                >
                                    使用已配置的私有 Registry 认证检查更新
                                </div>

                                <!-- Registry 选择器 -->
                                <div
                                    v-if="privateRegistries.length > 0"
                                    class="space-y-2"
                                >
                                    <select
                                    ...
                                    >
                                        ...
                                        <option
                                            v-for="reg in privateRegistries"
                                            :key="reg.host"
                                            :value="reg.host"
                                        >
                                            {{ reg.name || reg.host }}
                            ...
                            <div
                                v-if="selectedImage?.sourceType === 'private'"
                                class="flex-shrink-0"
                            >
                                <svg
                                    class="w-5 h-5 text-primary-600 dark:text-primary-400"
                    ...
                    >
                        <div
                            class="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg flex-shrink-0"
                        >
                            <svg
                                class="w-5 h-5 text-gray-600 dark:text-gray-400"
                                fill="none"
                        ...
                        <div
                            v-if="selectedImage?.sourceType === 'local'"
                            class="ml-auto flex-shrink-0"
                        >
                            <svg
                                class="w-5 h-5 text-primary-600 dark:text-primary-400"
            ...
            <div class="card w-full max-w-md p-6 animate-scale-in" @click.stop>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button
                        ...
                        :disabled="
                            updatingTagIds.has(selectedImage?.id) ||
                            !newTagInput.trim()
                        "
                        class="btn btn-primary"
                    >
                        <svg
                            v-if="updatingTagIds.has(selectedImage?.id)"
                            class="w-4 h-4 animate-spin mr-1"
                            viewBox="0 0 24 24"
                        >
                            <circle
